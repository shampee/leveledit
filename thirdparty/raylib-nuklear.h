...
NK_API Color ColorFromNuklear(struct nk_color color);               // Convert a Nuklear color to a raylib Color
NK_API Color ColorFromNuklearF(struct nk_colorf color);             // Convert a Nuklear floating color to a raylib Color
NK_API Rectangle RectangleFromNuklear(struct nk_context * ctx, struct nk_rect rect); // Convert a Nuklear rectangle to a raylib Rectangle
    ...
    nk_foreach(cmd, ctx) {
        switch (cmd->type) {
            ...
            case NK_COMMAND_LINE: {
                ...
                Vec2f32 startPos = CLITERAL(Vec2f32) {(float)l->begin.x * scale, (float)l->begin.y * scale};
                Vec2f32 endPos = CLITERAL(Vec2f32) {(float)l->end.x * scale, (float)l->end.y * scale};
            ...
            case NK_COMMAND_CURVE: {
                ...
                Vec2f32 begin = CLITERAL(Vec2f32) {(float)q->begin.x * scale, (float)q->begin.y * scale};
                Vec2f32 controlPoint1 = CLITERAL(Vec2f32) {(float)q->ctrl[0].x * scale, (float)q->ctrl[0].y * scale};
                Vec2f32 controlPoint2 = CLITERAL(Vec2f32) {(float)q->ctrl[1].x * scale, (float)q->ctrl[1].y * scale};
                Vec2f32 end = CLITERAL(Vec2f32) {(float)q->end.x * scale, (float)q->end.y * scale};
            ...
            case NK_COMMAND_ARC: {
                ...
                Vec2f32 center = CLITERAL(Vec2f32) {(float)a->cx, (float)a->cy};
            ...
            case NK_COMMAND_ARC_FILLED: {
                ...
                Color color = ColorFromNuklear(a->color);
                Vec2f32 center = CLITERAL(Vec2f32) {(float)a->cx * scale, (float)a->cy * scale};
            ...
            case NK_COMMAND_TRIANGLE: {
                ...
                Vec2f32 point1 = CLITERAL(Vec2f32) {(float)t->b.x * scale, (float)t->b.y * scale};
                Vec2f32 point2 = CLITERAL(Vec2f32) {(float)t->a.x * scale, (float)t->a.y * scale};
                Vec2f32 point3 = CLITERAL(Vec2f32) {(float)t->c.x * scale, (float)t->c.y * scale};
            ...
            case NK_COMMAND_POLYGON: {
                ...
                Vec2f32* points = (Vec2f32*)MemAlloc((unsigned int)((size_t)(p->point_count + 1) * sizeof(Vec2f32)));
                unsigned short i;
            ...
            case NK_COMMAND_POLYLINE: {
                ...
                for (unsigned short i = 0; i < p->point_count - 1; i++) {
                    Vec2f32 start = {(float)p->points[i].x * scale, (float)p->points[i].y * scale};
                    Vec2f32 end = {(float)p->points[i + 1].x * scale, (float)p->points[i + 1].y * scale};
            ...
            case NK_COMMAND_TEXT: {
                ...
                if (font != NULL) {
                    Vec2f32 position = {(float)text->x * scale, (float)text->y * scale};
            ...
            case NK_COMMAND_IMAGE: {
                ...
                Vec2f32 origin = CLITERAL(Vec2f32) {0, 0};
